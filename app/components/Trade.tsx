...
export default function LeverageTrade() {
  ...
  const themeColor = isLong ? 'from-[#4EC9B0] to-[#3EACB4]' : 'from-[#e74c3c] to-[#e26888]'

  return (
    <div className="flex items-center justify-center min-h-screen pt-20 px-4 pb-8 bg-gradient-to-b from-gray-900 via-[#1a1a2e] to-black">
      <div className="absolute inset-0 bg-[url('/grid.svg')] bg-center [mask-image:linear-gradient(180deg,white,rgba(255,255,255,0))]"></div>
      <div className={`relative w-full max-w-sm bg-gray-900 rounded-2xl p-5 shadow-2xl border transition-colors duration-300 ${
        isLong ? 'border-[#3EACB4]' : 'border-[#e74c3c]'
      }`}>
        <div className={`absolute inset-0 bg-gradient-to-br ${themeColor} opacity-20 rounded-2xl blur-xl`}></div>
        <div className="relative z-10">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-white">Leverage Trade</h2>
            <Button variant="outline" size="sm" className="text-white hover:bg-white hover:bg-opacity-10">
              <Settings className="h-5 w-5" />
            </Button>
          </div>
          <div className="flex justify-center items-center space-x-3 mb-4">
            <span className={`text-sm font-bold ${isLong ? 'text-[#4EC9B0]' : 'text-gray-400'}`}>
              OoLong
            </span>
            <div 
              className={`relative w-12 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors duration-300 bg-gradient-to-r ${themeColor}`}
            ...
            <span className={`text-sm font-bold ${!isLong ? 'text-[#e74c3c]' : 'text-gray-400'}`}>
              OoShort
            </span>
          </div>
          <div className="space-y-4">
            ...
            <div className="bg-gray-800 rounded-xl p-3">
              ...
              <div className="flex items-center">
                ...
                <div className="relative ml-2">
                  <div 
                    className="flex items-center bg-gray-700 rounded-lg py-1.5 px-2 cursor-pointer hover:bg-gray-600 transition duration-200"
            ...
            <div className="flex justify-center">
              <div className={`bg-gray-800 rounded-full p-1.5 transition-colors duration-300`}>
                <ArrowDown className={`h-4 w-4 ${isLong ? 'text-[#3EACB4]' : 'text-[#e74c3c]'}`} />
              </div>
            </div>
            {/* To Section */}
            <div className="bg-gray-800 rounded-xl p-3">
            ...
            {/* Leverage Slider */}
            <div className="bg-gray-800 rounded-xl p-3">
              <Label className="text-xs text-gray-300 mb-2 block">Leverage: {leverage}x</Label>
              <Slider
                value={[leverage]}
            ...
            {/* Trade Button */}
            <Button 
              className={`w-full py-3 text-base font-bold text-white rounded-xl shadow-lg transform hover:scale-105 transition duration-200 bg-gradient-to-r ${themeColor}`}
            >
